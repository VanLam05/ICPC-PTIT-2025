...
inline void _VanLam_()
{
    ...
    auto SUCCESS = [&](int mxDis) -> bool
    {
        set<int> Y;
        for (int i = 0, j = 0; j < n; j++)
        {
            while (points[i].fi + mxDis <= points[j].fi)
            {
                Y.insert(points[i].se);
                i++;
            }

            auto biggerIT = Y.lower_bound(points[j].se + mxDis);
            if (biggerIT != Y.end())
            {
                return true;
            }

            auto smallerIT = Y.upper_bound(points[j].se - mxDis);
            if (smallerIT != Y.begin())
            {
                return true;
            }
        }
