...
double DP(int A, int B, int C, int steps)
{
    ...
    if (A)
        dp[A][B][C] += DP(A + 1, B, C, steps + 1) * (double)(A * 1.0 / (A + B + C));
    if (B)
        dp[A][B][C] += DP(A, B + 1, C, steps + 1) * (double)(B * 1.0 / (A + B + C));
    if (C)
        dp[A][B][C] += DP(A, B, C + 1, steps + 1) * (double)(C * 1.0 / (A + B + C));
...
inline void _VanLam_()
{
    ...
    cout << fixed << setprecision(6) << DP(A, B, C, 0) << '\n';
}
