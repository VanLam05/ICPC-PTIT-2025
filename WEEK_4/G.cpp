...
// floor_sum from AtCoder Library (đã chỉnh về kiểu i64/i128 cho an toàn)
static inline int floor_sum(int n, int m, int a, int b)
{
    // tính S = sum_{i=0}^{n-1} floor((a*i + b)/m), yêu cầu m > 0
    int ans = 0;
    while (true)
    {
        if (a >= m)
        {
            ans += (i128)(n - 1) * n / 2 * (a / m);
            a %= m;
        }
        if (b >= m)
        {
...
inline void _VanLam_()
{
    ...
    int bad = floor_sum(Kmax + 1, D, C, A) - floor_sum(Kmax + 1, D, B, A - 1);
    int good = Kmax - bad;
